package com.wang.blog.web.exceptions;
...
public class DefaultExceptionHandler implements HandlerExceptionResolver {
	...
	public ModelAndView resolveException(HttpServletRequest request,
		...
		ex.printStackTrace();
		if (ex instanceof IllegalArgumentException || ex instanceof IllegalStateException || ex instanceof BaseException) {
		...
		if (isAjax(handler)) {
			try {
				...
				response.getWriter().print(JSON.toJSONString(Result.exception(500, ret)));
			} catch (IOException e) {
				// do something
			}
