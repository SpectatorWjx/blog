...
<a href="${base}/users/${user.id}">
    <img class="${clazz}" src="<@resource src=user.avatar />" alt="头像"/>
</a>
</#macro>

<#-- 栏目名称显示 -->
<#macro showChannel row>
<span class="hidden-xs label label-default <#if (row.featured > 0)>channel_featured</#if> <#if (row.weight > 0)>channel_top</#if>">${row.channelName}</span>
</#macro>
